  ...
  onLoad(){
    ...
    if (app.globalData.System.screenHeight / app.globalData.System.screenWidth > 1.8 ){
      this.setData({
        ...
        bgImage: this.data.$state.assetsUrl + "share_bg_invite_x.png"
      })
    }
    else{
      this.setData({
        ...
        bgImage: this.data.$state.assetsUrl + "share_bg_invite.png"
      })
    }

    //请求生成二维码url
    this.POST(this.$url.bus.weiChatCode, {
      scene: `entrance=10010?merchantId=${this.data.$state.user.merchantId}`,
      ...
      isHyaline: false
        ...
        this.setData({
          showLoading: false,
          weiChatCode: "https://" + res.data.cdnHttp + '/' + res.data.fileName + "?imageView2/1/w/100/h/100/q/85"
        }, this.canvasDrow )
    })
