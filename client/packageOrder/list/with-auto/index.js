...
Page({
    ...
    getOrderList(reqData){
        ...
        this.POST(this.$url.orders.list,  reqData).then((res)=>{

            res.data.data= res.data.data || [];

            res.data.data= res.data.data.map(v=> {
                v.allAmountStr= (   this.getProptry(v, "tripAmount", 0) + 
                                    this.getProptry(v, "goodsAmount", 0)- 
                                    this.getProptry(v, "discountAmount", 0) 
                                ).toFixed(2);
                return v;
            });
